...
// We make a map with the neighbors of each button.
...
export function getCodeVariations(suggestedCode: string): string[] {
  const walk = (chars: string[]): string[] => {
    const [char, ...rest] = chars

    if (!rest.length) return [char, ...map[char]]

    const variants = [char, ...map[char]]

    const output: string[] = []

    variants.forEach((v) => {
      const subVariants = walk(rest)
      subVariants.forEach((sv) => output.push([v, ...sv].join('')))
    })

    return output
  }

  const characters = suggestedCode.split('')
  const variations = [...new Set(walk(characters))]
